...
3. یک «نود ایران» بسازید و **فرمان نصب جدید** را روی سرور ایران با کاربر دارای `sudo` اجرا کنید. عامل نسخهٔ `2.3` را نصب می‌کند و endpoint کنترل ثبت‌شده را دریافت می‌کند.
...
6. برای هر inbound که می‌خواهید منتشر شود، در Railway یک **TCP Proxy دوم و مستقل** بسازید. target/internal port این Proxy باید دقیقاً «پورت انتشار» Rathole در فرم باشد، مثلاً `443`؛ نه `23333` و نه پورت local 3x-ui در ایران. hostname و پورت خارجی تازه‌ای که Railway می‌دهد را در بخش «TCP Proxy سرویس» وارد کنید. هرگز hostname:port مربوط به Control Proxy را در این فیلد تکرار نکنید.
7. سپس یک کانفیگ/لینک مربوط به inbound 3x-ui انتخاب کنید. `آدرس سرویس روی ایران` را `127.0.0.1` و `پورت محلی سرویس` را دقیقاً برابر پورت inbound فعال 3x-ui قرار دهید. اگر پنل `Connection refused` برای `127.0.0.1:<port>` نمایش می‌دهد، inbound روی آن پورت وجود ندارد یا Xray/3x-ui روی loopback گوش نمی‌دهد؛ قبل از ساخت tunnel این مورد را در خود 3x-ui اصلاح کنید.
8. در «دامنهٔ عمومی داخل Config» فقط نامی را وارد کنید که certificate آن واقعاً در Xray/3x-ui نصب شده است؛ پنل این نام را برای TLS/SNI می‌گذارد، اما socket کانفیگ و Ping فقط به hostname:port واقعی TCP Proxy سرویس Railway وصل می‌شوند. در نبود certificate معتبر، این فیلد را خالی بگذارید.
...
10. کارت تونل باید هم «TCP بیرون باز» و هم «3x-ui ایران آماده» را سبز نشان دهد. دکمهٔ Ping فقط TCP Proxy سرویس را می‌سنجد و اکنون استفادهٔ اشتباه از Control Proxy را با پیام مشخص رد می‌کند؛ برای وضعیت دوم، عامل 2.3 باید پس از چند ثانیه heartbeat جدید بفرستد.
11. اگر دامنهٔ شخصی وارد کرده‌اید، CNAME آن را به hostname `proxy_domain` نمایش‌داده‌شده در کارت tunnel بگذارید و آن رکورد را DNS-only نگه دارید؛ پورت اختصاصی Railway همچنان باید در لینک کلاینت حفظ شود.
...
اگر tunnel آنلاین نمی‌شود، ابتدا به‌ترتیب وضعیت «control proxy»، heartbeat نود، `rvg-rathole` و `rvg-rathole-agent` را روی نود بررسی کنید. سپس در کارت تونل، هر دو وضعیت «TCP بیرون باز» و «3x-ui ایران آماده» را بررسی کنید. سبزشدن Ping فقط پذیرش TCP توسط Railway را ثابت می‌کند و صحت TLS، SNI، UUID یا سرویس local را ثابت نمی‌کند. اگر پیام `Connection refused` برای `127.0.0.1:<port>` دیده می‌شود، پورت local انتخاب‌شده در فرم با inbound واقعی 3x-ui یکسان نیست یا inbound اجرا نشده است. اگر پیام «Control Proxy» دیده می‌شود، برای tunnel از همان TCP Proxy کنترل استفاده شده و باید یک TCP Proxy داده با target برابر `public_port` بسازید. Railway صراحتاً اعلام می‌کند که در دامنهٔ سفارشی، CNAME فقط hostname را تغییر می‌دهد و باید همان پورت تخصیص‌داده‌شدهٔ TCP Proxy استفاده شود.[3]
