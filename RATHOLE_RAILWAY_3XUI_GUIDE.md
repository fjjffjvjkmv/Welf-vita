...
پنل برای ساخت خودکار TCP Proxy به **Railway Account یا Workspace API Token** نیاز دارد. پس از deploy، در ابتدای صفحهٔ «تانلینگ» و بخش «دسترسی Railway برای TCP Proxy» توکن را یک‌بار وارد و ذخیره کنید. توکن با permission `0600` در volume پایدار `/data` ذخیره می‌شود و هرگز به مرورگر بازگردانده نمی‌شود. برای استقرار immutable می‌توان به‌جای UI متغیر `RAILWAY_API_TOKEN` یا `RAILWAY_TOKEN` را در Railway تنظیم کرد. توکن را هرگز داخل repository، اسکریپت نصب نود یا لینک subscription قرار ندهید.
...
1. در بخش «دسترسی Railway برای TCP Proxy»، یک Railway Account یا Workspace API Token را وارد و ذخیره کنید. badge باید «آماده» شود؛ اگر context ناقص بود، پنل را روی همان سرویس Railway deploy کنید و از وجود `RAILWAY_SERVICE_ID` و `RAILWAY_ENVIRONMENT_ID` مطمئن شوید.
2. در «مسیر کنترل Rathole»، گزینهٔ آماده‌سازی را اجرا کنید تا TCP Proxy کنترل ساخته شود.
3. یک «نود ایران» بسازید و **فرمان نصب جدید** را روی سرور ایران با کاربر دارای `sudo` اجرا کنید. عامل نسخهٔ `2.2` را نصب می‌کند.
...
6. یک کانفیگ/لینک مربوط به inbound 3x-ui انتخاب کنید. `آدرس سرویس روی ایران` را `127.0.0.1` و `پورت محلی سرویس` را دقیقاً برابر پورت inbound قرار دهید.
7. در «دامنهٔ عمومی داخل Config»، دامنه‌ای را وارد کنید که certificate آن در Xray/3x-ui نصب شده است. اگر دامنه ندارید، این فیلد را خالی بگذارید تا hostname تولیدشدهٔ Railway در لینک قرار گیرد.
8. «انتشار پورت و ساخت تانل» را بزنید. پنل یک TCP Proxy جدید می‌سازد و پورت خارجی Railway را در لینک client قرار می‌دهد.
9. اگر دامنهٔ شخصی وارد کرده‌اید، CNAME آن را به `proxy_domain` نمایش‌داده‌شده در کارت tunnel بگذارید و آن رکورد را DNS-only نگه دارید. سپس با دکمهٔ Ping، `دامنه:پورت` نهایی را بررسی کنید.
