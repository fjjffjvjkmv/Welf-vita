...
پنل برای ساخت خودکار TCP Proxy به credential از قبل پیکربندی‌شدهٔ Railway نیاز دارد. این credential را فقط از مسیر مدیریت امن پنل/متغیرهای محیطی موجود وارد کنید و هرگز داخل repository، اسکریپت نصب نود یا لینک subscription قرار ندهید.
...
1. در «مسیر کنترل Rathole»، گزینهٔ آماده‌سازی را اجرا کنید تا TCP Proxy کنترل ساخته شود.
2. یک «نود ایران» بسازید و **فرمان نصب جدید** را روی سرور ایران با کاربر دارای `sudo` اجرا کنید. عامل نسخهٔ `2.2` را نصب می‌کند.
3. منتظر بمانید تا heartbeat نود، وضعیت آنلاین و عامل فعال را نشان دهد.
4. در «تنظیمات Rathole»، گزینهٔ **Noise (رمزنگاری)** را انتخاب و ذخیره کنید.
5. یک کانفیگ/لینک مربوط به inbound 3x-ui انتخاب کنید. `آدرس سرویس روی ایران` را `127.0.0.1` و `پورت محلی سرویس` را دقیقاً برابر پورت inbound قرار دهید.
6. در «دامنهٔ عمومی داخل Config»، دامنه‌ای را وارد کنید که certificate آن در Xray/3x-ui نصب شده است. اگر دامنه ندارید، این فیلد را خالی بگذارید تا hostname تولیدشدهٔ Railway در لینک قرار گیرد.
7. «انتشار پورت و ساخت تانل» را بزنید. پنل یک TCP Proxy جدید می‌سازد و پورت خارجی Railway را در لینک client قرار می‌دهد.
8. اگر دامنهٔ شخصی وارد کرده‌اید، CNAME آن را به `proxy_domain` نمایش‌داده‌شده در کارت tunnel بگذارید و آن رکورد را DNS-only نگه دارید. سپس با دکمهٔ Ping، `دامنه:پورت` نهایی را بررسی کنید.
