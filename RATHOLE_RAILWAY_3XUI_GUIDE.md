...
8. در «دامنهٔ عمومی داخل Config» فقط نامی را وارد کنید که certificate آن واقعاً در Xray/3x-ui نصب شده است؛ پنل این نام را برای TLS/SNI می‌گذارد، اما socket کانفیگ همیشه به TCP Proxy Railway وصل می‌شود. در نبود certificate معتبر، این فیلد را خالی بگذارید.
...
10. کارت تونل باید هم «TCP بیرون باز» و هم «3x-ui ایران آماده» را سبز نشان دهد. دکمهٔ Ping فقط مورد اول را می‌سنجد؛ برای وضعیت دوم، عامل 2.3 باید پس از چند ثانیه heartbeat جدید بفرستد.
11. اگر دامنهٔ شخصی وارد کرده‌اید، CNAME آن را به `proxy_domain` نمایش‌داده‌شده در کارت tunnel بگذارید و آن رکورد را DNS-only نگه دارید.
