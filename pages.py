...
<section class="pg" id="pg-rathole">
  ...
  <div class="card" style="margin-top:16px" id="rathole-control-card">
    ...
    <div class="g2" style="margin:0">
      <div class="cm-note" style="margin:0"><i class="ti ti-info-circle"></i><span id="rh-control-note">برای کارکرد تونل، یک TCP Proxy روی پورت کنترل باید آماده باشد.</span></div>
      <div style="display:flex;justify-content:flex-end;align-items:center;gap:8px">
        <button class="btn btn-g btn-sm" onclick="bootstrapRatholeControl()"><i class="ti ti-player-play"></i> آماده‌سازی مسیر کنترل</button>
      </div>
    </div>
  </div>
  ...
  <div class="g2">
    ...
    <div class="card">
      <div class="card-title"><i class="ti ti-settings"></i> تنظیمات Rathole</div><div class="cm-note" style="margin-top:8px"><i class="ti ti-info-circle"></i><span>با تغییر پورت کنترل، Proxy کنترل Rathole بازسازی می‌شود و Nodeهای آنلاین کانفیگ جدید را می‌گیرند.</span></div>
      <div class="g2" style="margin:0">
  ...
  <div class="card" style="margin-top:16px">
    ...
    <div class="g2" style="margin:0">
      ...
      <div>
        ...
        <label style="margin-top:10px">آدرس سرویس روی ایران</label>
        <input class="cm-input" id="tun-local-host" type="text" value="127.0.0.1" placeholder="127.0.0.1 یا دامنه/hostname">
      ...
      <div style="grid-column:1/-1">
        ...
        <div class="g2" style="margin:10px 0 0">
          ...
          <div>
            <label>حالت DNS</label>
            <select class="cm-input" id="tun-dns-mode">
              <option value="manual_cname">CNAME به Public Endpoint</option>
              <option value="direct">مسیردهی مستقیم DNS</option>
            </select>
          </div>
        </div>
        <div class="g2" style="margin:10px 0 0">
          ...
          <div>
            <label>پورت تست</label>
            <input class="cm-input" id="tun-external-port" type="number" min="1" max="65535" value="443">
          </div>
        </div>
        <div class="g2" style="margin:10px 0 0">
          <div>
            <label>نوع endpoint</label>
            <select class="cm-input" id="tun-external-scheme">
...
function getTunnelFormSelection(){
  return {
    link: document.getElementById('tun-link-select')?.value || '',
    node: document.getElementById('tun-node-select')?.value || '',
    port: document.getElementById('tun-local-port')?.value || '443'
  };
}
function restoreTunnelFormSelection(sel){
  const linkSel=document.getElementById('tun-link-select');
  const nodeSel=document.getElementById('tun-node-select');
  const portEl=document.getElementById('tun-local-port');
  if(linkSel){
    const hasLink=[...linkSel.options].some(o=>o.value===sel.link);
    linkSel.value=hasLink?sel.link:'';
  }
  if(nodeSel){
    const hasNode=[...nodeSel.options].some(o=>o.value===sel.node);
    nodeSel.value=hasNode?sel.node:'';
  }
  if(portEl && sel.port) portEl.value=sel.port;
}
async function loadTunnelLinks(preserve=true){
  const previous=preserve?getTunnelFormSelection():{link:'',node:'',port:'443'};
  try{
    const r=await authF('/api/links');
    if(!r.ok) throw new Error(`links ${r.status}`);
    const d=await r.json();
    tunnelLinksCache=Array.isArray(d.links)?d.links:[];
    const sel=document.getElementById('tun-link-select');
    if(!sel)return;
    const activeLinks=tunnelLinksCache.filter(x=>!x.expired && x.active!==false);
    sel.innerHTML='<option value="">انتخاب کانفیگ...</option>'+activeLinks.map(x=>{
      const proto=(x.protocol||'').toUpperCase();
      return `<option value="${rEsc(x.uuid)}">${rEsc(x.label||x.uuid)} · ${rEsc(proto)}</option>`;
    }).join('');
    restoreTunnelFormSelection(previous);

    // Helpful default: one available config auto-selects, otherwise preserve user's choice.
    if(!sel.value && activeLinks.length===1){
      sel.value=activeLinks[0].uuid;
      applyTunnelPresetFromLink();
    }
    renderTunnelLinkInfo();
  }catch(e){
    console.warn('loadTunnelLinks:',e);
  }
}
function renderTunnelLinkInfo(){
  ...
  if(!x){
    info.textContent='یک کانفیگ آماده را انتخاب کن.';
    return;
  }
  const proto=String(x.protocol||'').toLowerCase();
  const suggested=proto.includes('mtproto')?(x.mtproto_port||443):443;
  const portEl=document.getElementById('tun-local-port');
  if(portEl && (!portEl.value || portEl.dataset.auto==='1')){
    portEl.value=suggested;
    portEl.dataset.auto='1';
  }
...
function renderTunnelNodeSelect(){
  ...
  const onlineNodes=ratholeCache.nodes.filter(n=>n.online);
  sel.innerHTML='<option value="">انتخاب نود...</option>'+ratholeCache.nodes.map(n=>
    `<option value="${rEsc(n.node_id)}" ${n.online?'':'disabled'}>${rEsc(n.label)} ${n.online?'· آنلاین':'· آفلاین'}</option>`
  ).join('');
  if(previous && onlineNodes.some(n=>n.node_id===previous)) sel.value=previous;
  else if(!sel.value && onlineNodes.length===1) sel.value=onlineNodes[0].node_id;
}
function applyTunnelPresetFromLink(){
  ...
  const x=tunnelLinksCache.find(v=>v.uuid===uid);
  if(!x){
    if(info)info.textContent='یک کانفیگ آماده را انتخاب کن.';
    return;
  }
  const proto=String(x.protocol||'').toLowerCase();
  const suggested=proto.includes('mtproto')?(x.mtproto_port||443):443;
...
async function bootstrapRatholeControl(){
  ...
  try{
    ...
    const d=await r.json();
    if(!r.ok) throw new Error(d.detail||'مسیر کنترل آماده نشد');
    toast(`مسیر کنترل آماده شد · ${d.host}:${d.port}`,'ok');
    await loadRathole();
  }catch(e){toast(e.message||'مسیر کنترل آماده نشد','err');}
}
function renderRatholeControlStatus(d){
  const badge=document.getElementById('rh-control-badge');
  const note=document.getElementById('rh-control-note');
  if(!badge||!note)return;
  if(d.server?.control_ready){
    badge.className='badge bg-green';
    badge.textContent='آماده';
    note.textContent=`Control Proxy: ${d.server.host}:${d.server.port} · پورت داخلی Rathole: ${d.server.control_port}`;
  }else{
    badge.className='badge bg-red';
    badge.textContent='آماده نیست';
    note.textContent=`پورت کنترل ${d.server?.control_port||23333} هنوز Proxy عمومی ندارد. ابتدا مسیر کنترل را آماده کن.`;
  }
}
async function createTunnelFromWorkflow(){
  // Capture current selection BEFORE any async operation.
  const nodeSel=document.getElementById('tun-node-select');
  const linkSel=document.getElementById('tun-link-select');
  const nodeId=nodeSel?.value||'';
  const uid=linkSel?.value||'';
  const port=Number(document.getElementById('tun-local-port')?.value||443);

  if(!uid && !nodeId){
    toast('اول کانفیگ آماده و نود ایران را انتخاب کن','warn'); return;
  }
  if(!uid){
    toast('اول کانفیگ آماده را انتخاب کن','warn'); return;
  }
  if(!nodeId){
  ...
  if(!node || !node.online){
    toast('نود انتخاب‌شده دیگر آنلاین نیست؛ صفحه را تازه می‌کنیم','warn');
    await loadRathole();
    return;
  }
  ...
  if(!link){
    toast('کانفیگ انتخاب‌شده دیگر در پنل موجود نیست؛ دوباره انتخاب کن','warn');
    await loadTunnelLinks(false);
    return;
  }
  ...
  try{
    const r=await authF('/api/rathole/tunnels',{
      ...
      body:JSON.stringify({
        node_id:nodeId,
        name:link.label||'Tunnel '+uid.slice(0,6),
        local_host:(document.getElementById('tun-local-host')?.value||'127.0.0.1').trim(),
        local_port:port,
        public_port:Number(document.getElementById('rh-public-port').value||443),
        protocol:isWs?'websocket':'tcp',
        websocket_path:'/',
        nodelay:true,
        link_id:uid,
        config_domain:(document.getElementById('tun-config-domain')?.value||'').trim(),
        origin_host:(document.getElementById('tun-origin-host')?.value||'').trim(),
        origin_port:Number(document.getElementById('tun-origin-port')?.value||443),
        dns_mode:(document.getElementById('tun-dns-mode')?.value||'manual_cname'),
    ...
    await loadRathole();
    restoreTunnelFormSelection(selected);
    setTimeout(()=>focusTunnelBinding(d.tunnel?.id),250);
  }catch(e){
    toast(e.message||'خطا در انتشار پورت','err');
  }
}
...
async function loadRathole(){
  try{
    ...
    document.getElementById('rh-server-port').dataset.saved=String(d.settings.server_bind_port);
    ...
    renderRatholeTunnels();
    await loadTunnelLinks(true);
    restoreTunnelFormSelection(workflowSelection);
    ...
    return `<div class="sr" style="padding:12px 0">
      <span class="sr-k">
        ...
        <small style="display:block;color:var(--dim);margin-top:3px">
          ${rEsc(n.hostname||'—')} · ${rEsc(n.platform||'—')}${n.last_seen_age!=null?` · آخرین heartbeat ${n.last_seen_age}s`:''}
        </small>
      </span>
      <span class="sr-v" style="display:flex;align-items:center;gap:7px;flex-wrap:wrap">
        ...
        ${n.online ? `<span class="badge ${n.rathole_client_active?'bg-green':'bg-amber'}">${n.rathole_client_active?'Rathole فعال':'Rathole در انتظار'}</span>` : ''}
 ...
 el.innerHTML=ratholeCache.tunnels.map(t=>{
   ...
   return `<div class="card" id="tunnel-row-${rEsc(t.id)}" style="padding:15px;margin:0">
     ...
     <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:12px;align-items:center">
       ...
       <button class="btn btn-g btn-sm" onclick="editTunnelRoute('${rEsc(t.id)}')"><i class="ti ti-world-cog"></i> تنظیم Route</button>
       <button class="btn btn-g btn-sm" onclick="bindTunnelExternal('${rEsc(t.id)}')"><i class="ti ti-plug-connected"></i> تست Endpoint</button>
       <button class="btn btn-p btn-sm" onclick="pingTunnel('${rEsc(t.id)}')"><i class="ti ti-activity"></i> Ping</button>
     </div>
   </div>`;
 }).join('');
}
async function editTunnelRoute(id){
  const t=(ratholeCache.tunnels||[]).find(x=>x.id===id);
  if(!t)return;
  const configDomain=prompt('دامنه‌ای که باید داخل Config نشان داده شود:', t.config_domain||'');
  if(configDomain===null)return;
  const originHost=prompt('دامنه/Host سرور خارج:', t.origin_host||'');
  if(originHost===null)return;
  const originPort=Number(prompt('پورت Origin خارج:', String(t.origin_port||443))||443);
  try{
    const r=await authF('/api/rathole/tunnels/'+encodeURIComponent(id)+'/route',{
      method:'PATCH',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({link_id:t.link_id,config_domain:configDomain,origin_host:originHost,origin_port:originPort,dns_mode:t.dns_mode||'manual_cname'})
    });
    const d=await r.json(); if(!r.ok)throw new Error(d.detail||'خطا');
    toast(d.tunnel?.dns_instruction?`Route ذخیره شد · ${d.tunnel.dns_instruction}`:'Route ذخیره شد','ok');
    await loadRathole();
  }catch(e){toast(e.message||'خطا در Route','err')}
}
...
async function saveRatholeSettings(){
 ...
 try{
   ...
   const d=await r.json();
   if(!r.ok){toast(d.detail||'خطا در ذخیره تنظیمات Rathole','err');return;}
   document.getElementById('rh-server-port').dataset.saved=String(newPort);
   if(d.bootstrap?.ok){
   ...
   toast(e.message||'خطا در ذخیره تنظیمات Rathole','err');
 }
}
